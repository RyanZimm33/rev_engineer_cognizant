...
from src.repositories.checkout_history_repository import SQLCheckoutHistoryRepository
from src.services.book_analytics_service import BookAnalyticsService
from src.services.book_generator_service import generate
...
def generate_seed_books(db: Session = Depends(get_db)):
    ...
    return 'Books where added to DB...'
...
def create_book(payload: BookCreate, db : Session = Depends(get_db)):
    repo = SQLBookRepository(db) 
    svc = BookService(repo)
    book = Book(**payload.model_dump())   #Pulls parameters from json payload, model_dump() is pydantic, converts it to dictionary
